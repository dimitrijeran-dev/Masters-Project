...
The interaction integral is the recommended primary upgrade because it restores 
path independence for heterogeneous materials and separates KI / KII while reusing 
the same domain integration structure.
...
- Auxiliary fields are the classical Williams asymptotic fields evaluated using
  tip-local material properties

Notes
-----
- The auxiliary displacement gradients are evaluated by stable finite
  differencing of the closed-form asymptotic displacement field. This is slower
  than a fully analytic implementation but is robust and easy to verify.
- The compliance-gradient correction is implemented in engineering-strain Voigt
  form, consistent with the rest of this repository.
"""
...
def S_matrix(E: float, nu: float, plane_stress: bool) -> np.ndarray:
    if plane_stress:
        return np.array(
            [[1.0 / E, -nu / E, 0.0], [-nu / E, 1.0 / E, 0.0], [0.0, 0.0, 2.0 * (1.0 + nu) / E]],
            dtype=float,
        )
...
def _mode_I_stress_local(local_x: np.ndarray, K: float = 1.0) -> np.ndarray:
    ...
    ct = np.cos(th)
    st = np.sin(th)
    sxx = srr * ct * ct - 2.0 * srt * st * ct + stt * st * st
    syy = srr * st * st + 2.0 * srt * st * ct + stt * ct * ct
...
def _rotate_stress_local_to_global(sig_local: np.ndarray, R: np.ndarray) -> np.ndarray:
    S_local = np.array([[sig_local[0], sig_local[2]], [sig_local[2], sig_local[1]]], dtype=float)
...
def _aux_stress_global(
    ...
    del E_tip, nu_tip, plane_stress  # stresses here are unit-K LEFM fields; material enters via E' in final conversion
    x_local = to_local(x_global, tip, R)
    if mode == "I":
...
# -----------------------------------------------------------------------------
# Inhomogeneity correction helpers
# -----------------------------------------------------------------------------

def E_gradient_at_gp(dN_dx: np.ndarray, E_nodes: np.ndarray) -> np.ndarray:
    return dN_dx.T @ np.asarray(E_nodes, dtype=float).reshape(-1)
...
def compute_J_domain_q4(
    ...
    for e in range(ne):
        ...
        E_e = float(E_by_elem[e])
        D = D_matrix(E_e, nu, plane_stress)
        E_nodes = np.full(4, E_e, dtype=float)
        ...
        for (xi, eta) in gps:
            ...
            term = np.array(
                [sxx * du_ds + txy * dv_ds - Wdens, txy * du_ds + syy * dv_ds],
                dtype=float,
            )
            J_standard += float(term @ gradq) * detJ

            if use_eischen_correction and E_elem is not None:
                dE_dx_vec = E_gradient_at_gp(dN_dx, E_nodes)
                dE_ds = float(dE_dx_vec @ crack_dir)
                J_correction += (-(Wdens / E_e) * dE_ds * q) * detJ
...
def compute_interaction_integral_q4(
    ...
    for e in range(ne):
        ...
        for (xi, eta) in gps:
            ...
            dE_dx_vec = E_gradient_at_gp(dN_dx, E_nodes) if E_elem is not None else np.zeros(2, dtype=float)
            dE_ds = float(dE_dx_vec @ crack_dir)
            dS_ds = dS_dE_matrix(E_e, nu, plane_stress) * dE_ds
...
if __name__ == "__main__":
    ...
    logging.info("Import this module and call compute_interaction_integral_q4(...) or compute_J_domain_q4(...)")
