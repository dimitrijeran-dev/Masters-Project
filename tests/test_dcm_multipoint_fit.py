...
def _cod_from_ki(ki: float, eprime: float, r: float) -> float:
    return (8.0 * ki / eprime) * math.sqrt(r / (2.0 * math.pi))
...
def test_estimate_plateau_ki_uses_robust_multipoint_fit():
    ...
    for i, r in enumerate(r_vals):
        cod = _cod_from_ki(ki_true, eprime, r)
    ...
    outlier_cod = 2.8 * _cod_from_ki(ki_true, eprime, 5.0e-4)
